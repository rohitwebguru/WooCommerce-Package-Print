...
class Wf_Woocommerce_Packing_List_Deliverynote
{
	...
	*/
	...
	{
		$arr[$this->module_base]=__('Delivery Note', 'wf-woocommerce-packing-list');
		return $arr;
	}
	...
	*/
	...
	public function add_admin_pages($menus)
	{
		$menus[]=array(
			'submenu',
			WF_PKLIST_POST_TYPE,
			__('Delivery Note','wf-woocommerce-packing-list'),
			__('Delivery Note','wf-woocommerce-packing-list'),
			'manage_woocommerce',
			$this->module_id,
	...
	public function add_print_actions($item_arr, $order, $order_id, $button_location)
	{
		if($button_location=='detail_page')
		{
			$item_arr[]=array(
				...
				'action'=>'',
				'label'=>__('Delivery note','wf-woocommerce-packing-list'),
				'tooltip'=>__('Print/Download Delivery note','wf-woocommerce-packing-list'),
				'is_show_prompt'=>0, //always 0
				'items'=>array(
					array(
						'action'=>'print_deliverynote',
						'label'=>__('Print','wf-woocommerce-packing-list'),
						'tooltip'=>__('Print Delivery note','wf-woocommerce-packing-list'),
						'is_show_prompt'=>0,
						'button_location'=>$button_location,
					),
					array(
						'action'=>'download_deliverynote',
						'label'=>__('Download','wf-woocommerce-packing-list'),
						'tooltip'=>__('Download Delivery note','wf-woocommerce-packing-list'),
						'is_show_prompt'=>0,
						'button_location'=>$button_location,
					)
				),
	...
	public function add_email_print_buttons($wt_actions, $order, $order_id, $email_obj, $sent_to_admin)
	{
        ...
        if(in_array('wc-'.$order->get_status(), $show_print_button_for))
        {
        	$wt_actions[$this->module_base]=array(
				'print'=>array(
					'title'=>__('Print Delivery note', 'wf-woocommerce-packing-list'),
					'email_button'=>true,
	...
	*/
    ...
    {
    	if($action=='print_deliverynote' || $action=='download_deliverynote')
    	{
	        ...
	        if(!is_null($this->customizer))
	        {
	        	...
	        	$this->customizer->template_for_pdf=($action=='download_deliverynote' ? true : false);
	        	$html=$this->generate_order_template($order_ids,$pdf_name);
				if($action=='download_deliverynote')
	        	{
